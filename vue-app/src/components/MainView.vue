<template>
  <div>
    ...
    <TaskVueIntro firstButton="Click me" secondButton="Open popup" />
  </div>
</template>
<script>
...
export default defineComponent({
  components: {
    MyBtn: MyBtn,
    MyPopup:
      MyPopup /*we can also write just Popup, then it would mean that the key and value are the same*/,
    TaskVueIntro,
  },
  props: {
  ...
  setup() {
    var clickCount =
      ref(
        0
      ); /*ref alows us to have reactivity - if something changes in javascript, then it also changes in the HTML*/
